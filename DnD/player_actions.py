...
def get_user_action_choice(choose_prompt : str, action_options : list[str], action_options_prefixes : list[str] | None = None, subtexts : list[str] | None = None, start_y : int = 0) -> str:
    """Have the user select an action for a list of actions\n
    Choose prompt is the string the player will see when prompted to choose. Recomended example: 'Choose action: '\n
    ...
    if CONSTANTS["use_fancy_item_selection"]:
        ...
        while True:
            action_nr : str[int] = input(choose_prompt)
